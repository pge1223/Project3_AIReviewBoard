...
## 2026-07-27 (이어서 2 — 점수 궤적 붕괴 원인 규명 + calibration 오발동 수정)

- 증상(사용자 배포 서버 재테스트, 수행보고서 v1.0~v1.3 동일 파일): 예전엔 29.5→49.87→49.2→
  57.01 상승세였는데 재테스트에서 29.5→61.66→62.67→49.5 — v1.1이 급등하고 최고 문서 v1.3이
  최저(실현 가능성 7.5/30 = 25% 상한).
- 원인 규명(전부 로컬 결정론 재현 — 업로드 PDF를 서버와 동일 경로(extract_document)로 파싱,
  build_score_cap 재계산):
  - v1.0(29.5=6+12+7.5+4)과 예전 v1.1(49.87=12+19.2+12+6.67)이 로컬 상한 계산과 **정확히
    일치** → 예전 곡선은 현행 코드의 상한 그대로. 재테스트의 v1.1(실현 19, 기술 23.33)은
    상한(12, 19.2)을 초과 — 코드는 dev와 동일(diff 없음 확인)이므로 코드 변화가 아니라
    **입력(rubric) 변화**.
  - ① **rubric description 드리프트**: 동적 rubric은 프로젝트마다 LLM 재추출(temp=0·seed는
    best-effort)이라 description 표현이 흔들리고, `_criterion_kind`가 description까지 섞어
    판별해 kind가 뒤집힘(예: 실현 가능성 desc에 "AI 모델" → kind=ai) → 섹션 매칭·S4 검사
    경로가 달라져 같은 파일·같은 코드에서 상한이 프로젝트마다 달라짐.
  - ② **S4 실체 어휘 누락**: v1.3의 추진 일정 섹션("4 단계 구분, 단계별 완료 기준 사전 정의,
    마일스톤·완료 기준, 검증 정확도 85% 이상")이 프로토타입·인프라 어휘가 없다는 이유로
    '실행 방법 미명시'(S4) 판정 → S3와 겹쳐 MULTI 25% 상한 → 실현 가능성 7.5/30.
- 수정(calibration.py, 최소 침습 2건):
  - `_criterion_kind`: **검증된 항목명(3중 검증으로 원문 고정) 우선** 판별, 항목명으로 알 수
    없을 때만 description 참고 — 드리프트가 상한에 못 미치게 차단.
  - feasibility 실체 어휘에 **"마일스톤"·"완료 기준"** 추가(미래형이면 기존대로 불인정).
    '단계별로 추진' 같은 막연한 표현은 추가하지 않음 — v1.1의 한 줄짜리 일정("3 단계로
    추진함")은 여전히 S4 상한(12/30) 유지.
- 검증: 실파일 재계산 — v1.0 **29.5 유지**, v1.1 **49.87(예전 곡선) 복원**, v1.3 **상한 전면
  해제**(위원 제안 그대로 = 최고점). 검토한 대안(초소형 섹션 시 전체 본문 폴백)은 v1.1의
  정당한 상한까지 풀어 61.66으로 만들기에 기각. 테스트 376 passed(신규 회귀 2: kind 항목명
  우선/마일스톤 일정 인정), 실패 7건은 기존 #174발 ideation(무관).
- 적용 조건: 머지·배포 후 **재분석**부터 반영(저장된 회의 점수는 소급되지 않음). 전체 곡선
  확인은 새 프로젝트에 v1.0→v1.3 순서 재업로드가 확실.

## 2026-07-27 (이어서 — 발표 자료 세트 확장: fig5·fig6 + 실험 6~8)

- 한 일(`docs/prompts/Comprehensive_Report/` — 기존 fig1~4가 07-26 오전까지만 반영하고 있어,
  그 이후의 표시 계층 결정론 작업을 같은 "전 수치 실측" 원칙으로 추가):
  - **`2026_07_27_시각화자료/`** (날짜별 폴더 규칙) — fig5 판정 표시 일관성(판정 밴드 + 실측 사례
    「적정」인데 6/15=40% → 항상 「보완 필요」), fig6 버전 추적 정확성(해결됨 표시 6→2 실측 +
    재표현 판별 Dice 0.82·0.64 흡수 / 0.55 구분, 임계 0.66).
  - **노트북 셀 추가**(종합리포트_설계검증_시각화.ipynb): 실측 6(판정 일관성)·실측 7(버전 추적)
    코드 셀 + 실측 8(2차 근거 게이트·정직 폴백 — 인용 원문 게이트/STEP1 정합/배타 배정/버전별
    스냅샷/분량 합본) 표 셀. 전 코드 셀 실행 검증(기존 fig1~4 재생성 포함, 오류 0).
  - **Before_After 실험 6~8 추가**, **설계가이드 5번째 축 '표시 일관성'(1.6) 추가** + 1.3에 인용
    원문 게이트(③) 반영 + 시각화 참조 경로 정정(html→ipynb, PNG 폴더 2곳).
- 결정/이유: 수치는 전부 devlog 실측 기록에서만 가져옴(지어낸 벤치마크 없음). 테스트 개수처럼
  시점에 따라 달라진 값은 새 자료에 다시 적지 않음(기존 문서의 366개는 작성 시점 기준 그대로 둠).
- 기타: 상세 화면 해결 탭 빈 상태 문구 "지적"→"해결" 분기(VersionTrackerTestPage.jsx, 해결 탭일 때만).

## 2026-07-27 (판정 일관성 + 지적-근거 정합 + 재표현 잔존 매칭 — 사용자 테스트 피드백 일괄 반영)

- 한 일(전부 표시 계층 결정론 — 저장 데이터 불변, 기존 회의에도 소급 적용):
  - **판정 표시 일관성**: 위원 LLM 원 판정은 상한(calibration) 적용 전 기준이라 "판정 적정인데
    15점 중 6점" 모순이 보이고, 재분석마다 판정이 흔들려 접속 시점마다 달라 보이던 문제 —
    화면 판정을 **최종(보정 후) 점수 비율**에서 결정론 도출(85%↑우수/65%↑적정/40%↑보완
    필요/미만 중대 리스크, reviewer_prompt 밴드와 동일 경계). 우선순위 정렬·팝업·STEP 3·점수
    변화 팝업이 한 곳에서 계산되어 일괄 일치.
  - **재표현 잔존 매칭**: 재분석마다 LLM이 같은 지적을 다른 문장으로 내면 백엔드 집합 비교가
    "이전 것 해결됨 + 같은 지적 신규"로 잘못 갈라 모순 표시("법·제도적 제약 설명 부족" 해결됨
    + "법적 제약·제도적 문제 고려 부족" 신규 동시 노출, 사용자 발견) — 지적문에서 일반어를
    뺀 **핵심 주제 토큰**(법·제도/예산/모델·알고리즘…)이 절반 이상 겹치면 잔존(보완 필요)으로
    표시하고 가짜 해결됨/신규 제거. 오합병 검증: "확산 계획"↔"성과 지표(KPI)"는 겹침 0.
  - **STEP 1 지적-근거 정합**: 제출 문서 인용이 항목 단위로 모여 지적과 무관한 문단이 붙던
    문제('법적 제약' 지적 밑 '구현 서비스 혁신성' 인용) — 지적 주제 토큰이 있는 인용만 표시
    (지적 전용 검증 인용은 항상), 없으면 "내용 부재" 정직 폴백.
  - **중심 자료 덤프 제거 + 보조 자료 정리**: 중심 자료의 원문 청크 인용(배점표 한 줄 덤프·
    심사 절차 안내)을 제거하고 파일명·배점·세부 기준만 유지. 보조 자료는 ① 배타 배정(섹션이
    의미상 가장 맞는 평가 항목에서만 — 'AI 모델·알고리즘 설계 적정성'은 기술성·혁신성으로,
    제외 항목도 배정 후보) ② 소제목만 있는 빈 청크 숨김 ③ 같은 파일·페이지 중복 블록 통합.
  - **청킹 개선 요청서**(`docs/rag_form_section_chunking_request.md`, 용준 앞): 서식 소제목과
    작성 요령 불릿이 청크로 분리되어 빈 블록/중복이 생기는 근본 원인 — 재현 경로 포함.
  - '중요한 정보' 탭을 AI 피드백 패널 맨 위로 이동(경이 위치 지정, `e9a1bd9`).
- 결정/이유:
  - 판정·잔존·정합 모두 **결정론 규칙**(비율 경계·토큰 매칭)으로 처리 — LLM 재호출 없이
    같은 데이터에 항상 같은 표시, 재분석 간 흔들림 제거.
  - 청킹은 용준(RAG) 영역이라 코드 수정 대신 요청서로 — 프론트 임시 방어(빈 블록 숨김·중복
    통합)는 청킹 개선 후에도 무해.

## 2026-07-26 (이어서 2 — 버전별 AI 피드백 스냅샷 + 분량 기준 전체 공고 자료 추출)

- 한 일:
  - **버전별 AI 피드백**(경이 요청): AI 피드백 탭이 어느 버전을 눌러도 최신 문서 검사 결과만
    보여주던 문제 — 원인은 수정본 업로드가 이전 target 문서를 지워 옛 버전 검사 데이터 자체가
    없던 것. 검사 결과(오탈자·맥락·분량밀도)를 **그 문서를 분석한 회의(=버전)에 스냅샷**으로
    저장(workbench.py `_snapshot_ai_feedback` — 최신 회의 document_id와 검사 대상 일치 시만),
    `/comparison` versions에 `ai_feedback` 전달(comparison.py), 프론트는 선택 버전의 스냅샷을
    렌더. 스냅샷 없는 옛 버전은 최신 데이터를 대신 보여주지 않고(오귀속) "기록 없음" 정직 안내
    — 이후 제출부터 버전별로 쌓임.
  - **분량 기준 전체 공고 자료 추출**: "최대 30p 이내"가 붙임2 서식에 있는데 기준 추출이 첫
    공고 문서(공고문)만 봐서 "기준 없음"이 나오던 문제(NIA 실측) — 모든 criteria 문서 합본
    (combine_criteria_documents 재사용)으로 확장 + 프롬프트에 수행보고서·붙임 서식 분량 규정
    포함 명시. 캐시 키를 `format_check_cache_v2`로 올려 옛 "기준 없음" 캐시 자동 재계산.
  - **표기**: "20p / 기준 기준 없음" 중복 수정 → "20p / 30p"(기준 있음) / "20p / 기준 없음".
    AI 피드백 탭에 '중요한 정보' 접힘 탭 추가(도표·시각화 자료 포함 권고 문구).
- 결정/이유:
  - 스냅샷은 회의 문서에 저장(문서 컬렉션이 아니라) — 수정본 업로드가 target 문서를 지우는
    기존 흐름을 바꾸지 않으면서 버전 기록을 보존. workbench.py(재인 영역) 수정은 스냅샷 훅과
    분량 기준 합본 두 지점만 최소 침습으로.
  - 검증: ai/meeting 379개 통과, vite 빌드 성공, 서버 PYTHONPATH 기준 import 확인.
    backend/tests 기존 실패는 변경 전과 동일(Mongo 연결 필요 환경 이슈)임을 stash 비교로 확인.
