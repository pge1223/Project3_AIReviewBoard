...
## 2026-07-30 (이어서 — 통합 리포트 단독 제출본 완성: TST-002 실측·3부 완결)

- **TST-002 위원 일관성 → 완료**: 로컬 실LLM 실행은 OPENAI_API_KEY quota 소진으로 막혔으나,
  배포 서버에서 동일 채점 코드·동일 문서를 독립 3회 반복 분석한 기존 실측이 정확히 TST-002
  요구임을 확인 — v1.0 편차 0(상한 지배 구간 완전 무편차) / v1.1 편차 3 / v1.2 편차 1,
  전부 사전 정의 허용범위(총점 10) 이내. 통합 리포트 1.4절 신설 + 추적표 ✅(이행률 9/9),
  관련 문서(트랙B·원고·슬라이드)의 "보류/계획" 문구 일괄 정리.
- **통합 리포트를 단독 제출본으로 완성** (폴더 하나 제출 가능):
  - 서두 "평가 대상 요약" 표(시스템·모델·데이터셋·테스트셋 — 강사 요구 5요소 커버) +
    아키텍처 그림(fig12 복사) + 핵심 설계 3원칙 압축
  - 3부(문서 검토)를 링크 요약 → 완결 섹션으로 확장: 골든셋×점수 결합 표, 29.5=상한 합
    재현 표, 실험 9 Before/After, Ragas·G-Eval 대응 표(실측값 포함)
  - 5장 트러블슈팅 개선 기록(6건 표) 신설, 종합 해석은 6장으로
- 표현 정리: 제출물 전체에서 "품질 조작" → "품질 단계별 구성" 등 중립 표현으로 통일(사용자 수정 반영).
- 최종보고서 파트원고는 강사 제출 대상 아님(팀 보고서용, 팀장 전달)을 확인 — 원고 고유
  내용(아키텍처 그림·설계 3원칙·트러블슈팅)은 통합 리포트로 흡수.

## 2026-07-30 (통합 평가 리포트 — 두 트랙 + A안 표기 원칙)

- 배경: 기존 평가 리포트가 문서 검토(종합 리포트) 트랙만 다뤄, 아이디어 회의 트랙까지
  포함하는 통합본을 요청받음. 또한 "LangChain도 만들었다"는 이전 표기가 사실과 달라
  (LangChain 체인은 용준 담당) 팀장 협의로 **A안** 확정: LangGraph(경이)를 정면에
  세우고 LangChain·RAG는 출처 표기 인용.
- 산출물 (`docs/evaluation/2026_07_29_통합평가리포트_mky/`):
  - `통합_평가리포트_두트랙_20260729_mky.md` — 0장 A안 표기 원칙(가이드의 LangChain
    요구 ↔ LangGraph 관계) / 1부 공통(LangGraph 파이프라인 표: 노드·LLM 콜·병렬·실패
    대응·실측 시간, 병렬 50% 단축, 실패 노드 재개, LLM 0콜 결정론 구간 / 용준 검색
    실측 인용 / 재현성) / 2부 아이디어 회의(22노드·15조건부 분기 실측, 오케스트레이션
    구조 지표 6종, 관련 테스트 343통과·8실패 정직 표기) / 3부 문서 검토 요약 /
    4장 요구사항 추적표(MTG-001~007·RPT-004/006 완료 — 검수 기준 대 검증 실측 1:1,
    TST-002 보류) / 5장 해석·한계
  - figA(파이프라인 노드별 지연·콜), figB(두 트랙 그래프 규모 + 병렬 효과)
- 재검토: 기존 산출물의 LangChain 언급 0건 확인(A안 위반 없음), 다만 "왜 LangGraph인가"
  방어 논리가 없어 최종보고서 파트 원고 2장에 관계 설명 단락 보강.

## 2026-07-29 (평가 리포트 제출 세트 + 최종 발표 자료)

- 상향 변별(#192) 배포 후 최종 검증: 버전 곡선 **29.5 → 67 → 65 → 73** — 완성 문서 +8
  분리, 부실 문서 하한 유지. v1.0의 29.5는 상한 합(6+12+7.5+4)과 정확히 일치함을 로컬
  결정론 재계산으로 재현(LLM 무관).
- **평가 리포트 제출 세트** (`docs/evaluation/`, 강사 제출용 — 파일명 `_mky` 표기):
  - `종합리포트_평가리포트_20260729_mky.md` — 시스템·모델 / 데이터셋·골든셋(품질 조작
    4버전, 결정론 특성 실측) / Ragas·G-Eval ↔ 자체 6축 지표 매핑 / 실측 4건 / 실험
    1~9 / 한계·재현 방법
  - `scripts/eval_report_signals_mky.py` — 테스트셋 특성·상한 발동을 서버와 동일
    함수로 실측(리포트 숫자의 출처)
  - `종합리포트_평가리포트_시각화_20260729_mky.ipynb` + `2026_07_29_시각화자료_mky/`
    (그림 5장, 실행 출력 임베드·재실행 가능)
  - 검색 레이어는 용준 하네스 실측 인용: Recall@5 0.944→1.000(타입 라우팅, 36케이스,
    검수 전 참고치, `reports/rag_eval_current_v6_36_*`)
- **최종 발표 자료** (`docs/prompts/Comprehensive_Report/`): `2026_07_28_시각화자료/`
  fig7~11(앵커링 전후·최종 곡선·상한 내역·rubric 스코핑·골든셋 특성 — fig1~6 번호 이어감),
  `최종발표_슬라이드구성안_20260728.md`(12장 + 그림 인덱스),
  `최종보고서_종합리포트_파트원고_20260728.md`(보고서 붙여넣기용 8절)

## 2026-07-28 (이어서 — 65점 앵커링 수리: 위원 프롬프트 상향 변별 규칙)

- 실측: 같은 프로젝트에서 서로 다른 문서 4개(v1.1 4.6k자 ~ v1.4 14.2k자, 예산·비교군·
  외부검증·실명 출처까지 보강한 개선본 포함)가 전부 총점 65로 수렴. 결정론 상한은 전부
  미발동(로컬 재계산)이라 상한 탓이 아니고, adequate 구간(65~84%)의 라운드 넘버 앵커
  (12/15 + 23/30 + 22/30 + 8/10 = 정확히 65)로 판정.
- 원인: reviewer_prompt.txt 엄정 채점 지침이 하향 규칙(근거 빈약 시 구간 상한 강제)만
  있고 상향 규칙이 없어, LLM이 좋은 문서에도 strong(85%+)을 쓰지 않고 adequate 중간값에
  습관적으로 앉음 — 나쁜 문서 변별(29.5)은 되는데 좋은 문서 변별이 안 됨.
- 수리: [상향 변별] 규칙 추가 — (a)실명 출처 (b)정량 실적 (c)완료형 실행+검증 (d)차별성
  비교 네 요건 중 셋 이상이면 strong(85%+) 강제, 둘이면 adequate 상단(75~84%).
  "요건 충족 시 중간값 고정 금지"를 명시. 과대 점수는 결정론 상한이 별도로 방지하므로
  안전 — 하향 변별(기존 규칙)은 그대로 유지.
...
- 경이 요청("각 점수마다 왜 그 점수인지 + 상한이 무슨 뜻인지"): 점수 변화(+N점) 팝업을
  산정 과정 전체가 보이게 상세화 —
  - **판정별 점수 구간 표**: 이 항목 배점 기준 실제 점수 구간(중대 리스크 0–40% …
    우수 85–100%)을 표시하고 최종 점수가 속한 구간을 ▶ 강조 (judgmentFromScore와
    동일 경계라 표시 모순 없음)
  - **위원 N명 평균 제안**: breakdown의 source_review_ids 수를 comparison.py versions에
    reviewers로 추가(저장 데이터에서 산출 — 기존 회의 소급) + 리포트 단독 경로도 배선
  - **상한 발동 시**: 상한 점수(배점의 %), 발동 신호 사유 목록(calibration.signals),
    "상한이란?" 설명(원문 규칙 검사·min 적용·결정론·보완하면 풀림) 명시. 미발동이면
    "위원 제안이 그대로 최종"을 명시
- 검증: 388 passed(비교·채점 포함), esbuild 통과.

## 2026-07-27 (이어서 7 — v1.3 급락 원인 확정: 빈 섹션 상한 오발동 수리)

- 진단(현재 파일 = OneDrive 7/26 22시 저장분, 서버와 파싱 글자수 2175/5975/9358/13527자
  정확 일치 — 완전 재현): v1.3 실현 가능성 상한 7.5/30(-11.8점, 총점 -11.5의 사실상 전부)의
  원인은 **「실현 가능성」 제목만 있고 본문이 0자인 섹션이 매칭**되어, 빈 문자열에
  S2(정량 없음)+S4(실체 없음)가 발동 → MULTI 25% 상한. 문서 품질 문제가 아니라
  대제목-소제목 구조에서 빈 body를 채점 재료로 쓴 오발동.
- 수리(calibration.py `_criterion_evidence_text`): 본문이 빈 섹션은 매칭 후보에서 제외
  (별칭 매칭·label_tokens 매칭 양쪽) — "섹션 못 찾음"과 동일하게 전체 본문 폴백.
- 검증(실파일 재계산): v1.3 실현 상한 해제(위원 제안 19.33 그대로 → 총점 ~63.0으로 최고점),
  v1.0 29.5 유지(14자 부스러기 섹션은 body 있어 불변 — 저품질 앵커 유지), v1.1/v1.2 불변.
  예상 곡선 29.5 → 60.33 → 62.67 → ~63.0 (상승 완성). 회귀 테스트 1건 추가, 388 passed.
- 메모: 이전에 검토했던 "초소형 섹션 전체 폴백(F2)"의 정밀판 — 그때는 임계 250자가 정당한
  상한까지 풀어 기각했고, 이번엔 "빈(0자) 섹션 = 판정 불능 입력"이라는 명백한 경계만 수리.

## 2026-07-27 (이어서 6 — 항목별 적정 위원만 채점 + 기획·개발 2위원 전환 준비)

- 배경(경이 확정): 화면은 기획/개발 위원 2그룹인데 내부 채점은 구 4위원 — 한 항목에
  비담당 위원 점수까지 평균되어 3명 평균(1/3 단위 점수)이 발생. 아이디어 회의(기획·개발
  전문가 2명)와 종합 리포트의 위원 서사도 어긋남.
- 이번 구현(제 영역만):
  - **배정된 위원만 집계**(transform.py): expected_criterion_ids 밖 항목은 근거 게이트를
    통과해도 점수·지적 완전 제외(레거시 flat 경로는 기존 유지). reviewer.py는 비담당
    항목의 RAG-004/005 콜백 호출 생략(비용 절약). 회귀 테스트 2건 추가.
  - **추출 배정 규칙**(meetings.py 프롬프트): 항목 세부 평가내용과 관련 있는 위원만
    1~2명(주 1 + 보조 0~1, 단수 secondary 스키마 유지) 배정, 무관 위원 금지.
  - RUBRIC_EXTRACTION_VERSION 7→8 (재추출로 새 배정 적용).
- **RAG(용준 영역) 수정 금지 확인**: iter_persona_criteria에 보조 리스트(3명) 확장을
  넣었다가 경이님 지시("RAG는 내 영역 아님")로 **원복** — 항목당 1~2명은 기존
  primary/secondary 스키마로 충분해 RAG 변경 없이 설계 확정. 이후 ai/rag는 요청서로만.
- **2위원 전환 요청서**(docs/committee_two_reviewers_request.md, 용준님 앞): 선행 조건
  2건(persona_cards domain_tags에 competition 추가, role_mapping에 두 위원 매핑) + 반영
  후 경이가 committee 템플릿 교체 후속 PR. 색인 속도는 용준님이 1차(배치·스레드 환경변수)
  진행 중이라 요청서 종결.
- 검증: 387 passed(신규 2 포함), 실패 9건은 기존 ideation(무관), py_compile 통과.

## 2026-07-27 (이어서 5 — 서버 로그 실측(B) + rubric 캐시 버그 수리)

- 서버 로그 실측(윤한 제공, journalctl): 분석 1회 총 29~41초 = rubric 확정 7~11s +
  RAG 근거 검색 4~6s + 위원 검토 LLM 13~27s(gpt-4o-mini 4명 **병렬 확인**, 호출당 11~20s,
  completion 토큰 수가 지배) + 위원장 종합 10~16s(백그라운드 — 로딩바 무관). **사용자 체감
  최대 성분은 분석이 아니라 업로드 시 색인**: KURE 임베딩이 문서당 20~90초(청크당 ~1.1-1.3s,
  CPU) — 용준(RAG) 영역, 실측 숫자로 전달 예정.
- **rubric 캐시 영구 미스 버그 수리**: 같은 프로젝트 4연속 분석에서 rubric 확정이 매번
  ~9초 → 캐시 판정(meetings.py)은 버전 ≥7을 요구하는데 저장(rubric.py)은 3을 하드코딩 —
  "저장(3) < 요구(7)"가 항상 참이라 캐시가 영원히 무효, 매 분석마다 LLM 재추출(~9초+비용).
  상수를 `RUBRIC_EXTRACTION_VERSION`(rubric.py) 한 곳으로 단일화(meetings.py는 import,
  graph/__init__ export 추가, 하드코딩 assert 테스트도 상수 참조로). 부수 효과: 프로젝트 안에서
  rubric이 완전 고정되어 재추출 드리프트(판정 흔들림의 원천) 자체가 사라짐. 26 passed.
- 점수 미변(v1.1 58.33) 관련: Actions 배포 시각(10:13/10:20)과 서버 프로세스 교체 시각
  (11:26~11:47 사이)이 어긋남 — deploy.yml의 `git pull --no-rebase`가 스쿼시 머지 이력/서버
  로컬 변경과 충돌해 실패해도 스크립트가 계속 진행("성공" 표시)되는 구조. 서버 코드 스테일
  의심 → 윤한님께 `git log -1` 확인 요청(스테일이면 fetch+reset --hard 제안, 윤한 판단).

## 2026-07-27 (이어서 4 — 분석 로딩바 단계 세분화: 5% 고정 해소)

- 증상(사용자): 분석 로딩바가 5%에서 한참 멈춰 보임. 원인 — 진행률 토큰이 rubric 추출·
  RAG 근거 검색이 **다 끝난 뒤에야** 등록돼, 가장 오래 걸리는 준비 구간 내내 GET 폴링이
  빈 응답을 받음(가장 느린 구간이 통째로 "5%"로 뭉쳐 보임).
- 수정(경이 승인 하에 가은 progress 코드 확장 — 가은님께 공유 필요):
  - backend(meetings.py): 토큰을 분석 초입에 조기 등록 + 준비 단계를 stage 문자열로 세분화
    ("평가 기준 추출" → "근거 검색" → 기존 "위원 검토"). reviews_total은 준비 단계에 0으로
    둬 기존 소비처 3곳(ReviewBoardPrototype/MentorSelection/VersionTracker)과 하위호환.
    조기 등록 후 실패 경로(committee 400, 근거 검색 예외)에서 토큰 pop 누수 방지.
  - frontend(ReviewBoardPrototype.jsx): overallPercent에 준비 단계 매핑(추출 6%/검색 10%/
    검토 15~70%), 로딩 문구를 단계별 표시("공고문에서 평가 기준 추출 중" 등, 위원 검토는
    n/m명 완료).
- 참고: 실제 소요 시간 단축은 서버 로그([analyze] 단계별 elapsed) 실측 후 별도 진행 —
  병목이 근거 검색이면 용준(서비스 내부 병렬화 + 이벤트 루프 블로킹 해소), LLM이면
  모델 경량화(가은 검토) 영역. 위원 병렬·위원장 백그라운드 분리는 기존에 이미 적용됨.
- 검증: py_compile·esbuild 통과. 구/신 조합(배포 시차) 모두 기존 동작 유지 확인(신
  백엔드+구 프론트 = 기존 5% 동작, 구 백엔드+신 프론트 = 기존과 동일 문구).
- 추가(사용자 요청): 채점 완료(score_done) 시점에 로딩바 85% 정지 대신 **100% 완료** 표시 —
  그 시점에 "피드백 확인하기" 버튼이 이미 활성이라 사용자 기준 분석 완료인데 바만 85%에
  멈춰 있었음(위원장 종합은 백그라운드로 이어져 리포트 화면에서 채워짐). 위원 검토 구간을
  15~85%로 확장해 단절 없이 진행.

## 2026-07-27 (이어서 3 — STEP 1 지적-근거 정합 강화: 지적 전용 인용도 주제 검사)

- 증상(사용자 발견): v1.0 실현 가능성의 '법적 제약·예산' 지적 밑 STEP 1에 목표 부합성
  소속 문장("구체적인 수치 목표는 추후에 정할 예정")이 근거로 표시 — 위원 LLM이 다른
  항목의 문장을 지적 인용으로 붙인 사례. 원문 게이트는 존재만 검증하고 주제는 못 거름.
- 수정(VersionTrackerTestPage.jsx, 표시 계층):
  - STEP 1 주제 토큰 필터의 **fromRef(지적 전용 검증 인용) 면제 제거** — 지적 전용
    인용도 동일하게 주제 정합 검사. 걸러서 안 남으면 기존 "내용 부재" 폴백.
  - 검증 중 2차 결함 발견: 접미 제거가 1회뿐이라 '구체적인'→'구체적'에서 멈춰 불용어
    ('구체')에 못 닿음 → 무관한 지적·인용이 '구체적' 같은 일반어로 이어져 필터 통과
    (fromRef 면제만 없애면 실측 사례가 여전히 통과했음). **접미 반복 제거**(변화 없을
    때까지) + **2자 미만이 되는 제거 금지** 가드('제도'의 '도'까지 벗겨 주제가 사라지는
    과잉 제거 방지).
- 검증: 실측 2건 + 정상 유지 5건 + 재표현 잔존 매칭 회귀 4건(법·제도 pair 유지, 확산↔KPI
  구분 유지) = 11건 전부 통과(node 재현 스크립트), esbuild 문법 통과.
